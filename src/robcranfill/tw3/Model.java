...
public static final Color	PEG_COLORS[]		= { Color.red, Color.blue, new Color(128, 128, 128),
		Color.lightGray						};
public static final int		STANDARD_BOARD_SIZE	= 24;	// 24 in cannonical game

// public instance vars
//
public transient iGUI					GUI_;
...
private static Obstruct		obstructions_[][]	= 
	{ 
		{ // theta=0
			new Obstruct(-1, -1, 2), new Obstruct(-1, -2, 2), new Obstruct( 0, -2, 2), 
			new Obstruct( 0, -2, 3), new Obstruct( 0, -1, 1), new Obstruct( 0, -1, 2), 
			new Obstruct( 0, -1, 3), new Obstruct( 0, -3, 3), new Obstruct(-1,  0, 1) 
		},
		{ // theta=1
			new Obstruct(-1, -1, 2), new Obstruct(0, -2, 3), new Obstruct(0, -1, 2),
			new Obstruct( 0, -1, 3), new Obstruct(0,  1, 0), new Obstruct(1, -2, 3), 
			new Obstruct( 1, -1, 2), new Obstruct(1, -1, 3), new Obstruct(1,  0, 0)
		}, 
		{ // theta=2
			new Obstruct(-1, 1, 1), new Obstruct(0, -1, 3), new Obstruct(0, 1, 0),
			new Obstruct( 0, 1, 1), new Obstruct(0,  2, 0), new Obstruct(1, 0, 3),
			new Obstruct( 1, 1, 0), new Obstruct(1,  1, 1), new Obstruct(1, 2, 0)
		}, 
		{ // theta=3
			new Obstruct(-1, 0, 2), new Obstruct(-1, 1, 1), new Obstruct(-1, 2, 1),
			new Obstruct( 0, 1, 0), new Obstruct( 0, 1, 1), new Obstruct( 0, 1, 2),
			new Obstruct( 0, 2, 0), new Obstruct( 0, 2, 1), new Obstruct( 0, 3, 0)
		}
	};
...
public void newGame()
	...
	for (int i = 1; i < boardSize_ - 2; i++)
		{ // don't bother with corners
...
public void registerListener(iGUI theGUI)
	{
	GUI_ = theGUI;
	}
 ...
 **/
public void setPegAt(int c, int r, int player)
	{
	pegs_[c][r] = player;
	touch();
	}
 ...
 **/
public int getPegOwner(int c, int r)
	{
	return pegs_[c][r];
	}
 ...
 * Is c,r open?
 **/
public boolean pegIsOpen(int c, int r)
	{
	return (pegs_[c][r] == OPEN_PEG);
	}

/**
 * Is c,r OK for player? Player 0 moves vertically, so left- rightmost columns are unavailable to them. Player 1 moves horizontally, so top and bottom rows are unavailable.
 **/
 ...
 **/
public boolean pegBelongsTo(int c, int r, int player)
	{
	return (pegs_[c][r] == player);
	}

public boolean[] getLinksForPeg(TWPoint p)
	{
	return linkArray_[p.x][p.y];
	}

/**
 * Can we link from p1 to p2? It's assumed player owns both points - we're only checking for correct geometry and obstructing links here.
 **/
public boolean canLinkTo(TWPoint p1, TWPoint p2, int player)
	{
...
/**
 * Add the link to the lists. There are two lists - could they be consolidated? probably... One is for redrawing the board; the other for checking for win.
 **/
public void setLink(TWPoint startPoint, TWPoint endPoint, int direction, int owner)
	{
