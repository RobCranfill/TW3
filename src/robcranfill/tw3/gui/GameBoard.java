...
//    System.out.println("model stamp is " + ourModel_);
    ...
    //
    Vector<Link> links = ourModel_.getLinks();
...
/**
    Draw a link at the given location.
    These are in game coordinates, not screen coords.
**/
private void
drawLink(Graphics g, Link link) {
...
/**
    ...
    Could be done more prettily - icon? bitmap?
    These are in game coordinates, not screen coords.
**/
private void
drawPegAt(Graphics g, int r, int c) {
    ...
    if (pegOwner == Model.OPEN_PEG) { // a little x
...
/**
    Given an x,y coordinate from a mouseclick, 
    what row & col most closely correspond?
**/
public TWPoint
mapClickToRowAndCol(TWPoint clickPoint) {
